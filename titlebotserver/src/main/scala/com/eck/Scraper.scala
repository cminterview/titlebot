...
object Scraper {
  ...
  final case class GeneratePageInfo(url: String, replyTo: ActorRef[Supervisor.Command], databaseActor: ActorRef[Database.StorePageInfo], database: MongoService, router: ActorRef[GetPageInfoResponse]) extends Command
    ...
    Behaviors.setup[Command] { context =>
      ...
      Behaviors.receiveMessage {
        case GeneratePageInfo(url, replyTo, databaseActorRef, database, router) =>
          context.log.info(s"generating for ${url}")
